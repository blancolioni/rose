...
package body Rose.Interfaces.Executable.Server is
   ...
   procedure Handle_Get_Executable_Interface (Parameters :
   ...
   begin
      ...
      Rose.Server.Register_Handler
        (Server_Context,
         ...
         Handle_Get_Executable_Interface'Access);
      if not Instanced then
         Launch_Cap := Rose.Server.Create_Endpoint (Launch_Endpoint);
         Rose.Server.Create_Anonymous_Endpoint (Executable_Interface);
      end if;
      Rose.Server.Register_Handler
        (Server_Context,
         Launch_Endpoint,
   ...
   -------------------------------------
   -- Handle_Get_Executable_Interface --
   -------------------------------------
   ...
   end Handle_Get_Executable_Interface;
   ...
   procedure Handle_Launch (Parameters :
      in out Rose.Invocation.Invocation_Record) is
      Image : constant Rose.Capabilities.Capability := Parameters.Caps (0);
      Store : constant Rose.Capabilities.Capability := Parameters.Caps (1);
      ...
      Result : constant Rose.Capabilities.Capability := Local_Launch
         (Parameters.Identifier, Image, Store, Caps (1 .. Cap_Count));
   begin
      Rose.System_Calls.Initialize_Reply (Parameters, Parameters.Reply_Cap);
   ...
   begin
      Local_Launch := Launch;
      Launch_Cap := Rose.Server.Create_Endpoint (Launch_Endpoint);
      Published_Cap := Rose.Server.Create_Endpoint (Executable_Interface);
      Rose.Server.Register_Handler
        (Server_Context,
         Executable_Interface,
         Handle_Get_Executable_Interface'Access);
